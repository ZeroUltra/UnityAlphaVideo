...
public class Pic2Video
{
    ...
    /// <returns></returns>
    private string GetPicType(string picFileName)
    {
        int index = picFileName.IndexOf('0');
        int lastindex = picFileName.LastIndexOf('.');
        return (picFileName.Substring(0, index) + $"%{lastindex - index}d" + picFileName.Substring(lastindex, picFileName.Length - lastindex));
    }
